...
import org.apache.kafka.coordinator.transaction.{TransactionLogConfig, TransactionMetadata, TransactionState, TransactionStateManagerConfig, TxnTransitMetadata, TransactionLog}
              ...
              for (record <- batch.asScala) {
                ...
                TransactionLog.readTxnRecordKey(record.key) match {
                  ...
                  case transactionalId: String =>
                    ...
                    val txnMetadata: Optional[TransactionMetadata] = TransactionLog.readTxnRecordValue(transactionalId, record.value)
                    if (txnMetadata.isEmpty) {
                      loadedTransactions.remove(transactionalId)
                    } else {
                      loadedTransactions.put(transactionalId, txnMetadata.get())
                    }
                  case other =>
