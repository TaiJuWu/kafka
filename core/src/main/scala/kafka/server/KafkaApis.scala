  ...
  override def handle(request: RequestChannel.Request, requestLocal: RequestLocal): Unit = {
    ...
    try {
      ...
      request.header.apiKey match {
        ...
        case ApiKeys.CREATE_ACLS => maybeForwardToController(request, null)
        case ApiKeys.DELETE_ACLS => maybeForwardToController(request, null)
        ...
        case ApiKeys.ELECT_LEADERS => maybeForwardToController(request, null)
        case ApiKeys.INCREMENTAL_ALTER_CONFIGS => handleIncrementalAlterConfigsRequest(request)
        case ApiKeys.ALTER_PARTITION_REASSIGNMENTS => maybeForwardToController(request, null)
        case ApiKeys.LIST_PARTITION_REASSIGNMENTS => maybeForwardToController(request, null)
        case ApiKeys.OFFSET_DELETE => handleOffsetDeleteRequest(request, requestLocal).exceptionally(handleError)
        case ApiKeys.DESCRIBE_CLIENT_QUOTAS => handleDescribeClientQuotasRequest(request)
        case ApiKeys.ALTER_CLIENT_QUOTAS => maybeForwardToController(request, null)
        case ApiKeys.DESCRIBE_USER_SCRAM_CREDENTIALS => handleDescribeUserScramCredentialsRequest(request)
        case ApiKeys.ALTER_USER_SCRAM_CREDENTIALS => maybeForwardToController(request, null)
        case ApiKeys.UPDATE_FEATURES => maybeForwardToController(request, null)
  ...
  def handleOffsetCommitRequest(
    ...
    if (!authHelper.authorize(request.context, READ, GROUP, offsetCommitRequest.data.groupId)) {
      ...
      if (authorizedTopicsRequest.isEmpty) {
        ...
        throw KafkaApis.unsupported("Version 0 offset commit requests require zookeeper.")
      } else {
